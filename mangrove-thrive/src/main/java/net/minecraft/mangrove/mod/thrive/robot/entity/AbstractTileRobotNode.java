...
public abstract class AbstractTileRobotNode extends TileEntity implements ITileUpdatable,IUpdatePlayerListBox {
	...
	public final void update() {
        ...
        if( controlPosition==null){
            ...
            if( !MGThriveBlocks.robot_kernel.isPowered(worldObj, controlPosition)){
                doStop();
                return;
		...
		if (this.worldObj.isRemote) {
			...
			default:
//			    if( tick % 128==0){
//                    System.out.println("Client Side Stage="+stage);
//                    fireLifecycleEvent();                   
//                }
				break;
			}
...
//		System.out.println(this.worldObj + ": Client? handleClientUpdate:"+ stage + ":" + data);
		if (data.has("step")) {
...
//	    System.out.println(this.worldObj + ": Server? handleServerUpdate:" + stage + ":" + data);
		if (data.has("stage")) {
			if (!data.get("stage").isJsonNull()) {
				this.stage = Lifecycle.valueOf(data.get("stage").getAsString());
			}
