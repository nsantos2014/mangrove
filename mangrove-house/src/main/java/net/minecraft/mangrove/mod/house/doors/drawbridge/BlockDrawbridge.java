...
//	public static final PropertyEnum HALF = PropertyEnum.create("half", DoorHalf.class);
...
//				 .withProperty(HALF, DoorHalf.BOTTOM)
		);
...
//            boolean flag = state.getValue(HALF) == DoorHalf.TOP;
            ...
            switch (enumfacing) {
			case NORTH:
				this.setBlockBounds(0.0F, 0.8125F, -1.0F, 1.0F, 1.0F, 1.0F);
				break;
			default:
...
//            iblockstate = iblockstate.withProperty(HALF, hitY > 0.5F ? DoorHalf.TOP : DoorHalf.BOTTOM);
    ...
    {
        if (disableValidation) return true;
        return blockIn.getMaterial().isOpaque() && blockIn.isFullCube() || blockIn == Blocks.glowstone || blockIn instanceof BlockSlab || blockIn instanceof BlockStairs;
    }
