...
public class ItemMGBoat extends Item {
	private String name="mgboat";

	public ItemMGBoat() {
		this.maxStackSize = 1;
		GameRegistry.registerItem(this, name, MGFreightForge.ID);
	...
	public ItemStack onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn) {
		...
		if (movingobjectposition == null) {
			...
			boolean flag = false;
			float f9 = 1.0F;
			List list = worldIn.getEntitiesWithinAABBExcludingEntity(playerIn, playerIn.getEntityBoundingBox().addCoord(vec32.xCoord * d3, vec32.yCoord * d3, vec32.zCoord * d3)
			...
			if (flag) {
				...
				if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
					...
					EntityMGBoat entityboat = new EntityMGBoat(worldIn, (double) ((float) blockpos.getX() + 0.5F), (double) ((float) blockpos.getY() + 1.0F),
